...
/// struct for typed errors of method [`delete_test_lead`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteTestLeadError {
    Status400(models::ErrorResponse),
    Status401(models::InlineObject1),
    Status404(),
    UnknownValue(serde_json::Value),
...
pub async fn delete_test_lead(
    configuration: &configuration::Configuration,
    form_id: &str,
    account_id: &str,
    lead_id: Option<&str>,
) -> Result<models::DeleteTestLead200Response, Error<DeleteTestLeadError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_form_id = form_id;
    let p_query_account_id = account_id;
    let p_query_lead_id = lead_id;
    ...
    req_builder = req_builder.query(&[("accountId", &p_query_account_id.to_string())]);
    if let Some(ref param_value) = p_query_lead_id {
        req_builder = req_builder.query(&[("leadId", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DeleteTestLead200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DeleteTestLead200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DeleteTestLeadError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

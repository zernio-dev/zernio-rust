...
pub enum CreateInboxConversationError {
    ...
    Status409(models::CreateInboxConversation409Response),
    Status422(models::CreateInboxConversation422Response),
    Status429(models::CreateInboxConversation429Response),
    Status502(models::WhatsAppTemplateLookupError),
    DefaultResponse(models::WhatsAppTemplateLookupError),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`search_inbox_conversations`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub enum SendInboxMessageError {
    ...
    Status409(),
    Status422(),
    Status429(models::SendInboxMessage429Response),
    Status500(),
    Status502(models::SendInboxMessage429Response),
    Status503(models::ErrorResponse),
    DefaultResponse(models::WhatsAppTemplateLookupError),
    UnknownValue(serde_json::Value),
}
...
pub async fn search_inbox_conversations(
    ...
    limit: Option<i32>,
    cursor: Option<&str>,
) -> Result<models::SearchInboxConversations200Response, Error<SearchInboxConversationsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let uri_str = format!("{}/v1/inbox/conversations/search", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SearchInboxConversations200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SearchInboxConversations200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SearchInboxConversationsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

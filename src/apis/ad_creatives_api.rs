...
/// struct for typed errors of method [`list_ads_tik_tok_identities`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListAdsTikTokIdentitiesError {
    Status400(models::ErrorResponse),
    Status401(models::InlineObject1),
    Status404(models::ErrorResponse),
    Status409(models::ErrorResponse),
...
pub async fn list_ads_tik_tok_identities(
    configuration: &configuration::Configuration,
    account_id: &str,
    ad_account_id: &str,
) -> Result<models::ListAdsTikTokIdentities200Response, Error<ListAdsTikTokIdentitiesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_account_id = account_id;
    let p_query_ad_account_id = ad_account_id;

    let uri_str = format!("{}/v1/ads/tiktok-identities", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListAdsTikTokIdentities200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListAdsTikTokIdentities200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListAdsTikTokIdentitiesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

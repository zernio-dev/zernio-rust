...
/// struct for typed errors of method [`list_ads_instagram_posts`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListAdsInstagramPostsError {
    Status400(models::ErrorResponse),
    Status401(models::InlineObject1),
    Status403(),
    Status404(models::ErrorResponse),
...
pub async fn list_ads_instagram_posts(
    configuration: &configuration::Configuration,
    account_id: &str,
    ad_account_id: Option<&str>,
    ...
    limit: Option<i32>,
    after: Option<&str>,
) -> Result<models::ListAdsInstagramPosts200Response, Error<ListAdsInstagramPostsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_account_id = account_id;
    let p_query_ad_account_id = ad_account_id;
    let p_query_ig_user_id = ig_user_id;
    let p_query_limit = limit;
    let p_query_after = after;

    let uri_str = format!("{}/v1/ads/instagram-posts", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if let Some(ref param_value) = p_query_ig_user_id {
        req_builder = req_builder.query(&[("igUserId", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_limit {
        req_builder = req_builder.query(&[("limit", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListAdsInstagramPosts200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListAdsInstagramPosts200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListAdsInstagramPostsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

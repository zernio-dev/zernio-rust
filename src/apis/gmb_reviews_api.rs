...
/// struct for typed errors of method [`get_google_business_review`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetGoogleBusinessReviewError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
...
pub async fn get_google_business_review(
    configuration: &configuration::Configuration,
    account_id: &str,
    review_id: &str,
    location_id: Option<&str>,
) -> Result<models::GetGoogleBusinessReview200Response, Error<GetGoogleBusinessReviewError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_account_id = account_id;
    let p_path_review_id = review_id;
    ...
    let uri_str = format!(
        "{}/v1/accounts/{accountId}/gmb-reviews/{reviewId}",
        configuration.base_path,
        accountId = crate::apis::urlencode(p_path_account_id),
        reviewId = crate::apis::urlencode(p_path_review_id)
    );
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetGoogleBusinessReview200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetGoogleBusinessReview200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetGoogleBusinessReviewError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

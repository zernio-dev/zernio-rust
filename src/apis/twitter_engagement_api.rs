...
/// struct for typed errors of method [`get_tweet`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_tweet(
    ...
    tweet_id: &str,
    id: Option<&str>,
) -> Result<models::GetTweet200Response, Error<GetTweetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_account_id = account_id;
    let p_query_tweet_id = tweet_id;
    let p_query_id = id;

    let uri_str = format!("{}/v1/twitter/tweet", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetTweet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetTweet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetTweetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,

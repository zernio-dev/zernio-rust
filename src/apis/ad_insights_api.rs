...
/// struct for typed errors of method [`get_tik_tok_smart_plus_material_report`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTikTokSmartPlusMaterialReportError {
    Status400(models::ErrorResponse),
    Status401(models::InlineObject1),
    Status404(models::ErrorResponse),
    Status409(models::ErrorResponse),
...
pub async fn get_tik_tok_smart_plus_material_report(
    configuration: &configuration::Configuration,
    account_id: &str,
    ad_account_id: &str,
    start_date: String,
    end_date: String,
    level: Option<&str>,
    smart_plus_ad_ids: Option<&str>,
    ad_group_ids: Option<&str>,
    page: Option<i32>,
    page_size: Option<i32>,
) -> Result<
    models::GetTikTokSmartPlusMaterialReport200Response,
    Error<GetTikTokSmartPlusMaterialReportError>,
> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_account_id = account_id;
    let p_query_ad_account_id = ad_account_id;
    ...
    let p_query_smart_plus_ad_ids = smart_plus_ad_ids;
    ...
    if let Some(ref param_value) = p_query_smart_plus_ad_ids {
        req_builder = req_builder.query(&[("smartPlusAdIds", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_ad_group_ids {
        req_builder = req_builder.query(&[("adGroupIds", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetTikTokSmartPlusMaterialReport200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetTikTokSmartPlusMaterialReport200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetTikTokSmartPlusMaterialReportError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,

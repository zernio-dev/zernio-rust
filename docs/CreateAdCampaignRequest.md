...
**buying_type** | Option<**BuyingType**> | Meta only. OMIT on non-Meta platforms (Google Ads, etc.): any value is rejected with a 400. On Meta the server applies AUCTION when omitted, so it does not need to be sent; RESERVED = Reach & Frequency. SKAdNetwork app promotion requires AUCTION. (enum: AUCTION, RESERVED) | [optional]

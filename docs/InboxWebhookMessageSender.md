...
**whatsapp_username** | Option<**String**> | WhatsApp only. User's WhatsApp username (e.g. `@jane`). Not a stable identifier, because users can change it. Useful for display, not recommended as an identity anchor.  | [optional]
**instagram_profile** | Option<[**models::InboxWebhookMessageSenderInstagramProfile**](InboxWebhookMessageSenderInstagramProfile.md)> |  | [optional]

...
**since** | Option<**String**> | Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn's archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only). |  |
**until** | Option<**String**> | Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since. |  |

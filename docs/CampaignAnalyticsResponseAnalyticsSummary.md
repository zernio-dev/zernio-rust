...
**search_impression_share** | Option<**f64**> | Google only. Date-range ratio, not a percentage. Null when unavailable; Google's threshold sentinel values are preserved. | [optional]
**search_budget_lost_impression_share** | Option<**f64**> | Google only. Date-range ratio, not a percentage. Null when unavailable; Google's threshold sentinel values are preserved. | [optional]
**search_rank_lost_impression_share** | Option<**f64**> | Google only. Date-range ratio, not a percentage. Null when unavailable; Google's threshold sentinel values are preserved. | [optional]
**search_top_impression_share** | Option<**f64**> | Google only. Date-range ratio, not a percentage. Null when unavailable; Google's threshold sentinel values are preserved. | [optional]
**search_absolute_top_impression_share** | Option<**f64**> | Google only. Date-range ratio, not a percentage. Null when unavailable; Google's threshold sentinel values are preserved. | [optional]

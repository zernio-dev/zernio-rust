...
**conversion_events** | Option<[**Vec<models::ListConversionDestinations200ResponseDestinationsInnerConversionEventsInner>**](ListConversionDestinations200ResponseDestinationsInnerConversionEventsInner.md)> | OpenAI Ads only: the conversion event settings wired to this pixel. A `goal: conversions` create on POST /v1/ads/create optimizes for the first `optimizable` one; when none is, it returns 400.  | [optional]

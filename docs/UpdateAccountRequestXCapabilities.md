...
------------ | ------------- | ------------- | -------------
**analytics** | Option<**bool**> | Enable periodic analytics reads (impressions, likes, etc.) for this X account. Each X API call is metered as `posts_read` and billed pass-through (~$0.005/call at the time of writing; the actual rate depends on X's pricing tier).  | [optional]

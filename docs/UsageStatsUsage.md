...
**x_api_calls_by_operation** | Option<**std::collections::HashMap<String, i32>**> | Metronome users only. Per-operation X API call counts keyed by operation (e.g. `posts_read`, `content_create`). Resolve each key to price and metadata via `GET /v1/billing/x-pricing`.  | [optional]
